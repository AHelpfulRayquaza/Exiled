...
exports.commands = {
    ...
    set: function (target, room, user) {
		...
		if (!target) return this.parse('/pteam help');
		let parts = target.split(',');
		let mon = parts[1].trim();
		let slot = parts[0];
		if (!parts[1]) return this.parse('/pteam help');
    ...
    give: function (target, room, user) {
		...
		if (!target) return this.errorReply('USAGE: /pteam give [USER]'); /*
    ...
    take: function (target, room, user) {
		if (!this.can('broadcast')) return false;
		if (!target) return this.errorReply('USAGE: /pteam take [USER]'); /*
		let user = target.toLowerCase().trim(); */
    ...
    help: function (target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox('<center><b>Profile Team Commands<br>' +
				...
				'<code>give</code>: Global staff can give users ability to set their own team.<br>' +
				'<code>take</code>: Global staff can take users ability to set their own team.<br>' +
